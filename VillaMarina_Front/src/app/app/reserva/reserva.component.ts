...
export class ReservaComponent implements OnInit {
  ...
  ReservadoMensaje: boolean = false;
  ReservadoMensaje30: boolean = false;
  ...
  async ngOnInit() {
    ...
    if (this.config) {
      ...
      if (nowDate) {
        this.Today = new Date(new Date(nowDate).getTime() + new Date(nowDate).getTimezoneOffset() * 60000);
  ...
  Calendar() {
    this.CalendarDay = [];
    let TotalLastMonthDay = new Date(this.CalendarYear, this.CalendarMonth, 0).getDate();
    let TotalThisMonthDay = new Date(this.CalendarYear, this.CalendarMonth + 1, 0).getDate();
    let StartWeek = new Date(this.CalendarYear, this.CalendarMonth, 1).getDay();
    let EndWeek = new Date(this.CalendarYear, this.CalendarMonth + 1, 1).getDay();
    for (let i = TotalLastMonthDay - StartWeek + 1; i <= TotalLastMonthDay; i++) this.CalendarDay.push([i, this.CalendarMonth - 1, this.CalendarYear]);
    for (let i = 1; i <= TotalThisMonthDay; i++) this.CalendarDay.push([i, this.CalendarMonth, this.CalendarYear]);
    for (let i = 1; i <= 7 - EndWeek; i++) this.CalendarDay.push([i, this.CalendarMonth + 1, this.CalendarYear]);
  }
  ...
  SelectDay(item: Array<number>) {
    ...
    if (this.SelectedYear < this.SelectedYear2 || this.SelectedMonth < this.SelectedMonth2 || (this.SelectedDay || 1) < (this.SelectedDay2 || 1)) {
  ...
  async CreatePay() {
    ...
    if (this.payUrl) {
      ...
      if (Expire) {
        ...
        this.payUrl = '';
        this.PayError = false;
      } else this.PayError = true;
    }
    const FinalDay = this.GetDay(this.SelectedYear, this.SelectedMonth, this.SelectedDay);
    const InicialDay = this.GetDay(this.SelectedYear2, this.SelectedMonth2, this.SelectedDay2);
    ...
    if (PaySession) {
      ...
      if (Paying) this.CheckPaying(Paying);
    } else this.PayError = true;
    this.isLoading = false
  }
  ...
  CheckPaying(res: any) {
    for (const item of res)
      if (this.minDay * 24 * 60 + this.Today.getHours() * 60 + this.Today.getMinutes() <= item.ExpireDate * 24 * 60 + item.ExpireHour * 60 + item.ExpireMin)
