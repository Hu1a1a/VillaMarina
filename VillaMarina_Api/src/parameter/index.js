...
router.get("/Get", async (req, res) => {
  con.query(Config, (err, result) => (err ? res.send({ msg: "error" }) : res.send(result[0])));
});
        ...
        if (err) {
          if (req.headers.authorization === result[0].Token) {
            ...
            con.query(UPDATE, (err) =>
              err ? res.send({ msg: "Error en el DDBB, contacte con el administrador." }) : res.send({ msg: "Correcto!" })
            );
          } else res.send({ msg: "Error en el Token, volver a iniciar sessión." });
        } else res.send({ msg: "Error en el DDBB, contacte con el administrador." });
      });
