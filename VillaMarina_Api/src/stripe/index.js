        ...
        if (statError) {
          ...
          try {
            ...
            for (var i = req.body.Inicial; i <= req.body.Final; i++) {
              sqlString =
                sqlString +
                `,(${i},${ExpireDate},${ExpireDay.getHours()},${ExpireDay.getMinutes()},"${session.url.toString().split("/pay/")[1].split("#")[0]}")`;
            }
...
router.get("/Paying", (req, res) => {
  ...
  con.query(DELETE, () => null);
  con.query(sqlPaying, (err, result) => res.json(result));
});
